...
export const Route = createFileRoute("/kulhad-bareilly")({
  head: () => ({
    ...
    links: [{ rel: "canonical", href: "https://www.kulhad.shop/kulhad-bareilly" }],
  }),
  component: BareillyPage,
});

function BareillyPage() {
  return (
    <SiteLayout>
      <section className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <h1 className="font-display text-4xl font-bold text-foreground">Kulhad Wholesale Bareilly</h1>
        <p className="mt-3 text-muted-foreground">Bareilly (Uttar Pradesh) — mitti ke kulhad wholesale aur bulk order. Bareilly ke mashhoor bazaars aur cafe culture — restaurants, dhabas aur shaadi planners ke liye time par kulhad supply. Moradabad factory se approx 110 km — delivery 3-4 din me.</p>
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-6">
          <h2 className="font-display text-xl font-bold text-foreground">Aas-paas ke areas — Bareilly</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {[{ n: "Shahjahanpur", to: "/products" }, { n: "Pilibhit", to: "/products" }, { n: "Rampur", to: "/kulhad-rampur" }].map((c) => (
        ...
        <div className="mt-6 rounded-2xl bg-primary p-6 text-center text-primary-foreground">
          <h2 className="font-display text-xl font-bold">Bareilly me kulhad wholesale order karein</h2>
          <p className="mt-2 text-sm">Factory-direct price + time par delivery. Abhi WhatsApp karein.</p>
          <a href={waLink("Hi Kulhad Factory, I'd like a wholesale kulhad quote for Bareilly. Please share pricing and delivery.")} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center justify-center rounded-full bg-white px-6 py-3 text-sm font-semibold text-primary">WhatsApp pe quote lein</a>
        </div>
      </section>
    </SiteLayout>
  );
