...
export const Route = createFileRoute("/kulhad-dhanora")({
  head: () => ({
    ...
    links: [{ rel: "canonical", href: "https://www.kulhad.shop/kulhad-dhanora" }],
  }),
  component: DhanoraPage,
});

function DhanoraPage() {
  return (
    <SiteLayout>
      <section className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <h1 className="font-display text-4xl font-bold text-foreground">Kulhad Wholesale Dhanora</h1>
        <p className="mt-3 text-muted-foreground">Dhanora (Uttar Pradesh) — mitti ke kulhad wholesale aur bulk order.</p>
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-6">
          <h2 className="font-display text-xl font-bold text-foreground">Aas-paas ke areas — Dhanora</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {["Amroha","Gajraula","Hasanpur"].map((n) => (
        ...
        <div className="mt-6 rounded-2xl bg-primary p-6 text-center text-primary-foreground">
          <h2 className="font-display text-xl font-bold">Dhanora me kulhad wholesale order karein</h2>
          <p className="mt-2 text-sm">Factory-direct price + time par delivery. Abhi WhatsApp karein.</p>
          <a href={waLink("Hi Kulhad Factory, I'd like a wholesale kulhad quote for Dhanora. Please share pricing and delivery.")} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center justify-center rounded-full bg-white px-6 py-3 text-sm font-semibold text-primary">WhatsApp pe quote lein</a>
        </div>
      </section>
    </SiteLayout>
  );
